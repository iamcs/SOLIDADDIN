...
namespace SwCSharpAddinByStanley
{
    ...
    public class SwAddin : ISwAddin
    {
        ...
        //当零件时，对零件实施操作。当装配体时对选择的对象实施操作
        //public delegate void DoforAll(Face2 faceTarget, Face2 faceCompare, IModelDoc2 modDoc);
        ...
        public dynamic RandomColorValues(dynamic materialPropertyValues)
        {
            ...
            if (dyeMode > 127)
            {
                red = (dyeMode-128) / 128 * (1 - red) + red;
                green = (dyeMode - 128) / 128 * (1 - green) + green;
                blue = (dyeMode - 128) / 128 * (1 - blue) + blue;
            }
            else if (dyeMode < 127)
            {
                red = red * dyeMode / 128;
        ...
        public void OpenRelvantFile()
        {
            ...
            fileName = fileName.Replace(" ","");//SolidWorks文件名去除空格
            ...
            foreach (string x in siblingNames)
            {
                ...
                //路径文件名去除空格
                readyName = readyName.Replace(" ","");
                //不等于本身、路径文件名包括SolidWorks文件名、SolidWorks文件名包括路径文件名
